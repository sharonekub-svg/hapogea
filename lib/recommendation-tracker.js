...
function normalizeResultRow(rawRow) {
  // Extract actualWinner from markets[] for Odds-API rows (not set as top-level field)
  ...
  // Score-based fallback for rows where isFinal/statusGroup=4 but no market result
...
function extractResults(feed) {
  ...
  // Normalize BEFORE filter so Odds-API rows (isFinal/markets) are recognized
