...
async function supabaseSelect(table, { columns = "*", limit = 500, orderBy = "id.desc" } = {}) {
  const url = `${SUPABASE_URL}/rest/v1/${table}?select=${columns}&limit=${limit}&order=${orderBy}`;
  ...
  if (supabaseEnabled()) {
    try {
      const [recommendations, match_results, daily_stats] = await Promise.all([
        supabaseSelect("recommendations", { limit: 500, orderBy: "id.desc" }),
        supabaseSelect("match_results",   { limit: 500, orderBy: "id.desc" }),
        supabaseSelect("daily_stats",     { limit: 90,  orderBy: "stat_date.desc" }),
      ]);
